...
fn run_assemble_command(path: &Path, output_dir: &Path) -> Output {
    let mut assemble_cmd = cargo_bin_cmd!("rnsc");
    assemble_cmd.arg("asm").arg(path).arg("-d").arg(output_dir);
...
fn verify_assembly_behavior(disassembled: &str, hash: &str, output: &Output, output_dir: &Path) {
...
fn test_integration(
    ...
    let assemble_output = run_assemble_command(&path, &output_dir);
    ...
    let (disassembled, hash) = process_assembly_result(&assemble_output, &output_dir);
    verify_assembly_behavior(&disassembled, &hash, &assemble_output, &output_dir);
